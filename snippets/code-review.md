---
task: Code Review
objective: Systematic review of branches, commits, or pull requests
focus: [Quality, Standards Adherence, Recursive Tracing, Actionable Feedback]
---

# Snippet: Emoji-Driven Code Review Protocol

Apply this protocol when reviewing branches, commits, or pull requests. Each comment must be prefixed with an emoji to signal intent and severity. This snippet works in conjunction with the standards in `./.ai/stacks/` and the recursive logic in `./.ai/protocols/recursive-exploration.md`.

## 1. Emoji Legend

Use this fixed set of emojis to classify every review comment. Place the emoji at the **start** of each comment.
...
3. **Standards Check:** Load the relevant `./.ai/stacks/` file and verify the diff against its rules (method size, typing, naming, test coverage).
...
## 4. Recursive Quality Gates

Cross-reference every diff against these checks (sourced from `./.ai/stacks/`):
